...
# global classifier to be used by other files
global human_voice_clf
human_voice_clf = None
...
def target_value( basename, target_file ):
...
def compute_avg_coeffs( coeff_output_file ):
    ...
    avg_dict['human'] = target_value(base_name, target_file)
...
def create_single_output( training_folder ):
    # write the header
    header_added = False
...
def create_classifier(tr_folder, tg_file ):
    global human_voice_clf
    global target_file
    ...
    total_data, targets = create_single_output(training_folder)
    df = pandas.read_csv(total_data, usecols=coefficients)
    clf = svm.SVC(C=200000000, tol=1e-10, cache_size=600, kernel='rbf', gamma='auto', class_weight='balanced')
    print targets
...
if __name__ == "__main__":
    ...
    create_classifier(training_folder, target_file)
